...
class CamelGameWindow(QWidget):
    def __init__(self):
        ...
        # Поле для ввода кол-ва поставленных фигур + поле для их координат
        ...
        self.spin_required_figures.setMaximum(100)
        
        # Поле из клеток для отображения решения
    ...
    def solve(self):
        try:
            ...
            answers = set()
            Fisrt_Solution = list()
            SolveChess.board_recursion(N, L, 0, 0, answers, placed_figure, 0, Fisrt_Solution)
            if answers != set():
...
class ChessBoard:
    ...
    def board_place(matrix, placed_figure):
        for x, y in placed_figure:
            ChessBoard.place_figure(x, y, matrix)
        return matrix
...
class SolveChess:
    ...
    def board_recursion(N, L, place_x, place_y, answers, answer, cnt, First_Solution):
        if cnt == L:
            ...
            if len(answers) == 1:
                matrix = ChessBoard.board_place([['0' for _ in range(N)] for _ in range(N)], remember_answer)
        ...
        for i in range(place_x, N):
            ...
            for j in range(begin_y, N):
                if not Figure.camel_move(i, j).intersection(answer) and (i, j) not in answer :
                    answer.add((i, j))
                    SolveChess.board_recursion(N, L, i, j, answers, answer, cnt + 1, First_Solution)
                    answer.discard((i, j))
